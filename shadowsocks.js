    ...
    async fetch(request,env) {
        try {
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                ...
                // NekoBox 订阅路径 /sub/2/UUID
                if (url.pathname.toLowerCase() === `/sub/2/${subPath.toLowerCase()}` || url.pathname.toLowerCase() === `/sub/2/${subPath.toLowerCase()}/`) {
                    const currentDomain = url.hostname;
                    const ssHeader = 's'+'s';
                    const ssLinks = cfip.map(cdnItem => {
                        let host, port = 443, nodeName = '';
                        ...
                        const ssConfig = `${method}:${password}@${host}:${port}`;
                        const ssNodeName = nodeName ? `${nodeName}-${ssHeader}` : `${ssHeader}`;
                        const pluginStr = `v2ray-plugin;mode=websocket;tls;host=${currentDomain};path=${validPath}/?ed=2560;tls;mux=0`;
                        const encodedPlugin = encodeURIComponent(pluginStr);
                        return `${ssHeader}://${ssConfig}?plugin=${encodedPlugin}#${ssNodeName}`;
                    });
